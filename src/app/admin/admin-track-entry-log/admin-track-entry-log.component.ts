...
@Component({
  selector: 'app-track-entry-log',
  standalone: true,
  imports: [CommonModule],
  templateUrl: './admin-track-entry-log.component.html',
  styleUrls: ['./admin-track-entry-log.component.css']
})
export class AdminTrackEntryLogComponent {
  entryLogs: EntryLog[] = [
  ...
  itemsPerPage: number = 6; // Number of logs per page
  ...
  deleteLog(logId: number): void {
    if (confirm('Are you sure you want to delete this log?')) {
      this.entryLogs = this.entryLogs.filter(log => log.log_id !== logId);
      // Update paginated logs and total pages if necessary
  ...
  updatePagination(): void {
    // Logic to update paginatedEntryLogs and totalPages based on the current page and entryLogs
  }
  previousPage(): void {
